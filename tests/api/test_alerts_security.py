...
def user_a(client):
    ...
    client.post("/api/v1/auth/register", json={"email": email, "password": "Password123!", "name": "User A"})
...
def user_b(client):
    ...
    client.post("/api/v1/auth/register", json={"email": email, "password": "Password123!", "name": "User B"})
    resp = client.post("/api/v1/auth/login", json={"email": email, "password": "Password123!"})
    token = resp.json()["token"]["access_token"]
    return {"headers": {"Authorization": f"Bearer {token}"}, "id": resp.json()["user"]["id"]}
...
def test_cannot_update_other_user_alert(client, user_a, user_b):
    ...
    resp_b = client.put(f"/api/v1/alerts/{alert_id}", json=update_payload, headers=user_b["headers"])
