...
from scalper_today.domain.entities import EconomicEvent, Alert, AlertCondition, AlertType, Importance
...
async def test_scheduler_matches_and_notifies(scheduler, mock_expo, mock_db_manager):
    ...
    # Mock Repositories
    with patch("scalper_today.infrastructure.database.EventRepository") as MockEventRepo, \
         patch("scalper_today.infrastructure.database.repositories.AlertRepository") as MockAlertRepo, \
         patch("scalper_today.infrastructure.database.repositories.DeviceTokenRepository") as MockDeviceRepo:
        ...
        MockDeviceRepo.return_value.get_by_user_id = AsyncMock(return_value=[MagicMock(token="token-123")])
...
async def test_scheduler_avoids_duplicate_notifications(scheduler, mock_expo, mock_db_manager):
    ...
    event = EconomicEvent(id="event-1", time=event_time, title="T", country="C", currency="C", importance=Importance.HIGH)
    alert = Alert(id="a1", user_id="user-1", name="N", conditions=[AlertCondition(AlertType.HIGH_IMPACT_EVENT, None)])
...
async def test_scheduler_ignores_far_future_events(scheduler, mock_expo, mock_db_manager):
    ...
    event = EconomicEvent(id="event-far", time=event_time, title="T", country="C", currency="C", importance=Importance.HIGH)
    alert = Alert(id="a1", user_id="user-1", name="N", conditions=[AlertCondition(AlertType.HIGH_IMPACT_EVENT, None)])
...
def test_condition_matching_logic(scheduler):
    high_event = EconomicEvent(id="1", time="1", title="T", country="United States", currency="USD", importance=Importance.HIGH)
    low_event = EconomicEvent(id="2", time="1", title="T", country="United States", currency="USD", importance=Importance.LOW)
    ...
    event_surprise = EconomicEvent(id="3", time="1", title="T", country="C", currency="C", importance=Importance.MEDIUM, actual="100", forecast="50")
    event_no_surprise = EconomicEvent(id="4", time="1", title="T", country="C", currency="C", importance=Importance.MEDIUM, actual="100", forecast="95")
