...
class ForexFactoryCalendarProvider(IEventScraper):
    ...
    async def _fetch_payload(self) -> Optional[Any]:
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            try:
                response = await self._client.get(
                    self._settings.forexfactory_calendar_url,
                ...
                logger.warning(
                    "ForexFactory calendar request non-200",
                ...
                last_exception = exc
                logger.warning("Timeout fetching ForexFactory calendar", extra={"attempt": attempt})
            except httpx.HTTPError as exc:
                last_exception = exc
                logger.warning(
                    "HTTP error fetching ForexFactory calendar",
                    extra={"attempt": attempt, "error": str(exc)},
                )
            except Exception as exc:  # pragma: no cover - defensive guard
                last_exception = exc
                logger.error(
                    "Unexpected error fetching ForexFactory calendar",
                    extra={"attempt": attempt, "error": str(exc)},
                )
        ...
        if last_exception:
            logger.error(
                "Failed to fetch ForexFactory calendar",
    ...
    @staticmethod
    def _extract_importance(raw: str) -> int:
        text = raw.strip().lower()
        if not text:
            return 2
        if "high" in text or "alto" in text:
            return 3
        if "medium" in text or "moderate" in text or "medio" in text:
            return 2
    ...
    def _build_event_id(event_dt: datetime, title: str, country: str, currency: str) -> str:
        ...
        event_id = f"ff-{time_part}-{location_slug}-{title_slug}"
        return event_id[:100]
